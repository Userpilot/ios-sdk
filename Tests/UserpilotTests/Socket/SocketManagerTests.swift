...
final class SocketManagerTests: XCTestCase {
    ...
    func testPublish_withValidEvent_shouldSendEvent() {
        ...
        mockSocketSubscription.onSocketEventSentCalled = { event, payload, message, status in
    ...
    func testPublish_withSpecificSubscription_shouldNotifySpecificSubscription() {
        ...
        specificSubscription.onSocketEventSentCalled = { event, payload, message, status in
            XCTAssertEqual(event, eventName)
            expectation.fulfill()
        }
        ...
        // Act
        socketManager.publish(eventName, payload: payload, shouldCloseSocket: false, socketSubscription: specificSubscription)
