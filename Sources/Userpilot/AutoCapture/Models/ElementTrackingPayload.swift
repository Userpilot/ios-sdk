...
internal struct ElementTrackingPayload {
    // MARK: - Properties
    ...
    func toDictionary() -> [String: Any] {
        var dict: [String: Any] = [
            ...
            AutoCaptureConstants.hierarchy: screenHierarchyPath,
            AutoCaptureConstants.positionIndex: positionIndex
        ]

        if let elementLabel = elementLabel {
            dict[AutoCaptureConstants.elementLabel] = elementLabel
        }
        if let accessibilityId = accessibilityId {
            dict[AutoCaptureConstants.accessibilityId] = accessibilityId
        }
